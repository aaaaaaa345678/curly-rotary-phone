...
新增模板：`templates/result.html`（推荐结果展示页）
...
@app.post('/recommend')
def recommend():
    # 传入 user_id, algo_name, results, watched_list
    return render_template('result.html', user_id=1, algo_name='User-Based CF',
                          results=[('Toy Story', 4.8)], watched_list=[('Fargo', 5)])
```
